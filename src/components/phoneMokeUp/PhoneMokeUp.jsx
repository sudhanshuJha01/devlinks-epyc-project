...
const data = useLinkDataStore(state=>state.data)
  ...
  return (
    <section className='flex flex-col items-center gap-14 justify-center w-[237px] h-[514px]' >
        ...
        {presentIndex==1?<RectangularComponent content={data[data.length-1]}/>:<RectangularComponent/>}
        {presentIndex==2?<RectangularComponent content={data[data.length-1]}/>:<RectangularComponent/>}
        {presentIndex==3?<RectangularComponent content={data[data.length-1]}/>:<RectangularComponent/>}
        {presentIndex==4?<RectangularComponent content={data[data.length-1]}/>:<RectangularComponent/>}
        </section>
    </section>
  )
}
